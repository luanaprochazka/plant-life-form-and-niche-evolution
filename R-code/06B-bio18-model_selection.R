...
# -----------------------------------------------------------
# AICc values and model plausibility - OUwie models - BIO18
# -----------------------------------------------------------
...
bio18 <- read.csv("bio18/bio18log_data.csv")

##Model Results: 

BM1   <- readRDS("bio18/bio18_fitBM1.rds")
BMS   <- readRDS("bio18/bio18_fitBMS.rds")
OU1   <- readRDS("bio18/bio18_fitOU1.rds")
OUM   <- readRDS("bio18/bio18_fitOUM.rds")
OUMA  <- readRDS("bio18/bio18_fitOUMA.rds")
OUMV  <- readRDS("bio18/bio18_fitOUMV.rds")
OUMVA <- readRDS("bio18/bio18_fitOUMVA.rds")
...
bio18_AICc <- cbind(SIMMAP, BM1_AICc, BMS_AICc, OU1_AICc, OUM_AICc, OUMA_AICc, OUMV_AICc, OUMVA_AICc)
bio18_AICc <- as.data.frame(bio18_AICc)
...
bio18_AICc$BM1_AICc[bio18_AICc$SIMMAP %in% BM1Results2$SIMMAP] <- "NA"
...
bio18_AICc$BMS_AICc[bio18_AICc$SIMMAP %in% BMSResults2$SIMMAP] <- "NA"
...
##Verify theta value (if theta value is between bio18 minimum and bio18 maximum == TRUE,if not == FALSE):

THETA.OU1 <- c()  
for (i in SIMMAP) {
  THETA1 <- all(OU1[[i]]$theta[,1] >= min(bio18$bio18.mean)) && 
            all(OU1[[i]]$theta[,1] <= max(bio18$bio18.mean))
...
bio18_AICc$OU1_AICc[bio18_AICc$SIMMAP %in% OU1Results2$SIMMAP] <- "NA"
...
THETA.OUM <- c()  
for (i in SIMMAP) {
  THETA1 <- all(OUM[[i]]$theta[,1] >= min(bio18$bio18.mean)) &&
            all(OUM[[i]]$theta[,1] <= max(bio18$bio18.mean))
...
bio18_AICc$OUM_AICc[bio18_AICc$SIMMAP %in% OUMResults2$SIMMAP] <- "NA"
...
THETA.OUMA <- c()  
for (i in SIMMAP) {
  THETA1 <- all(OUMA[[i]]$theta[,1] >= min(bio18$bio18.mean)) && 
            all(OUMA[[i]]$theta[,1] <= max(bio18$bio18.mean))
...
bio18_AICc$OUMA_AICc[bio18_AICc$SIMMAP %in% OUMAResults2$SIMMAP] <- "NA"
...
THETA.OUMV <- c()  
for (i in SIMMAP) {
  THETA1 <- all(OUMV[[i]]$theta[,1] >= min(bio18$bio18.mean)) &&
            all(OUMV[[i]]$theta[,1] <= max(bio18$bio18.mean))
...
bio18_AICc$OUMV_AICc[bio18_AICc$SIMMAP %in% OUMVResults2$SIMMAP] <- "NA"
...
THETA.OUMVA <- c()  
for (i in SIMMAP) {
  THETA1 <- all(OUMVA[[i]]$theta[,1] >= min(bio18$bio18.mean)) &&
            all(OUMVA[[i]]$theta[,1] <= max(bio18$bio18.mean))
...
bio18_AICc$OUMVA_AICc[bio18_AICc$SIMMAP %in% OUMVAResults2$SIMMAP] <- "NA"

bio18_AICc <- mutate_all(bio18_AICc, function(x) as.numeric(as.character(x))) 
...
path
write.csv(bio18_AICc, paste0(path, "bio18/bio18_AICc.csv"), row.names = F)

# -------------------------------------------
# White noise models and AICc values
# -------------------------------------------

bio18.WT <- readRDS(paste0(path, "Results/04-OUwie_and_WN_models/bio18/bio18_fitWN.rds"))

###AICc:

library(tidyverse) 

bio18_WT_AICc  <- as.vector(unlist(lapply(bio18.WT, function (x) x[[4]][[7]])))

bio18_AICc2 <- add_column(bio18_AICc, WT_AICc = bio18_WT_AICc, .after = 1)

write.csv(bio18_AICc2, paste0(path, "bio18/bio18_AICc_all.csv"), row.names = F)

# ------------------------------------------
# Delta AICc and model selection - bio18
# ------------------------------------------
rm(list = ls())

##set working directory to the  data-paper folder:

bio18.aicc <- read.csv("data-paper/Results/04-OUwie_and_WN_models/bio18_AICc_all.csv", row.names=1)

##Calc DELTA AICc: 

##bio18: 

bio18.D.AICc <- data.frame(matrix(ncol = 8, nrow = 2000))
x <- c("WN_DAICc", "BM1_DAICc", "BMS_DAICc", "OU1_DAICc", "OUM_DAICc", "OUMA_DAICc", "OUMV_DAICc", "OUMVA_DAICc")
colnames(bio18.D.AICc) <- x

for (i in 1:2000) { ## For each row in table: 
  
  min.i <- min(bio18.aicc[i,], na.rm = T) ## select the min value in the row
  
  bio18.D.AICc[i,] <- bio18.aicc[i,] - min.i ##subtract the min value of each element in the row
  
}

##Save Delta Aicc data:

write.csv(bio18.D.AICc, "data-paper/Results/04-OUwie_and_WN_models/bio18_Delta_AICc_all.csv", row.names = T)

###Calc the frequency at which each  model was best fitted:

#Compare with all WN model fits:

WN.DELTA <- drop_na(bio18.D.AICc, WN_DAICc)
...
BM1DELTA <- drop_na(bio18.D.AICc, BM1_DAICc)
...
BMSDELTA <- drop_na(bio18.D.AICc, BMS_DAICc)
...
OU1DELTA <- drop_na(bio18.D.AICc, OU1_DAICc)
...
OUMDELTA <- drop_na(bio18.D.AICc, OUM_DAICc)
...
OUMADELTA <- drop_na(bio18.D.AICc, OUMA_DAICc)
...
OUMVDELTA <- drop_na(bio18.D.AICc, OUMV_DAICc)
...
OUMVADELTA <- drop_na(bio18.D.AICc, OUMVA_DAICc)
...
write.csv(resultadoVF, "/data-paper/bio18/bio18_best_fit_all.csv", row.names = T)

# ------------------------------------------------------------
# Plot bio18 theta estimate values - Only the best models 
# ------------------------------------------------------------

rm(list = ls())
##set working directory to the  data-paper folder:

# The best model to bio18 is OUM model: 

OUM <- readRDS("/Results/04-OUwie_and_WN_models/bio18/bio18_fitOUM.rds")

##Delta AICc data: 

bio18.DAicc <- read.csv("/data-paper/bio18/bio18_Delta_AICc_all.csv")

##Remove OUM models with bad fit:

validos <- drop_na(bio18.DAicc, OUM_DAICc)
...
bio18.theta <- summarySE(thetavf2, measurevar= "theta", groupvars = "states", na.rm = T)
bio18.theta
...
bio18.theta$states <- factor(bio18.theta$states, levels=c("therophyte", "chamaephyte", "phanerophyte", "geophyte"))
...
grob <- grobTree(textGrob("(A)", x=0.03,  y=0.96, hjust=0,
                          gp=gpar(col="black", fontsize=8, fontface="bold")))


ggplot(data = bio18.theta, mapping = aes(x=states, y= theta)) + 
  geom_point(size = 1.5) +
  ...
  scale_y_continuous(name = "?? estimate in OUM models \n Precipitation of warmest quarter (mm)", limits = c(0, 700), n.breaks = 10) + 
...
ggsave(width = 4, height = 4, dpi = 600, filename = "bio18_OUM_theta_BEST.png")

#################################################

ggplot(data = thetavf2, mapping = aes(x=states, y= theta)) +   geom_dotplot(binaxis = "y", stackdir = "center", dotsize= 0.1) 
  geom_point(size = 1.5)
  geom_jitter(data = thetavf2, mapping = aes(x= states, y= theta, color = states), size = 0.4, alpha = 0.4, height = 0.00, width = 0.1) +
  scale_color_manual(values=c("#7743DB","#FFC93C","#B1AEB9","#DC267F")) +
  geom_errorbar(aes(ymin=theta-sd, ymax=theta+sd), width=.2) +
  geom_point(size = 1.5) +
  annotation_custom(grob) +
  scale_y_continuous(name = "?? estimate in OUM models \n Precipitation of warmest quarter (mm)", n.breaks = 10) + 
...
ggsave(width = 4, height = 4, dpi = 600, filename = "D:/bio18_OUM_theta_BEST.png")